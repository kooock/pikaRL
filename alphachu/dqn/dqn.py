...
class DQN:
    ...
    def _buildNetwork(self, last_layer_input_size=256, learning_rate=0.0001):
        with tf.variable_scope(self.network_name):
            ...
            with tf.name_scope(self.network_name + "_Conv_set_layer1"):
                filter1 = tf.Variable(tf.random_normal([5, 5, 4, 32], stddev=0.01))
            ...
            with tf.name_scope(self.network_name + "_Conv_set_layer3"):
                filter3 = tf.Variable(tf.random_normal([3, 3, 32, 64], stddev=0.01))
...
#Hack : fc!_W SIZE IS 6144
    ...
    def predict(self, state):
        ...
        q,s4 = self.session.run([self.q_predict], feed_dict={self._input: feed_state, self.dropout_late: 1.0})
